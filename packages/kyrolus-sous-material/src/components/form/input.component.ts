...
@Component({
  ...
  template: `
    ...
    <ng-template #labelTplt>
      @if(label()){
      <label [class]="labelClasses()" [for]="label()">
        {{ label() }}
        @if(isRequired()){
    ...
    <ng-template #_input>
      ...
      <input
        ...
        [attr.aria-describedby]="ria_describedby()"
        (input)="onInput($event)"
        (blur)="onBlur()"
        [value]="value()"
        [attr.appearance]="appearance()"
...
export class InputComponent implements ControlValueAccessor, OnInit {
  ...
  readonly ria_describedby = computed(() =>
  ...
  readonly leftIconClases = computed(() => {
    if (this.dir.direction() == 'ltr') {
      return 'br-r-tl-3 br-r-bl-3 br-w-r-1 br-r-only';
    } else {
      return 'br-r-tr-3 br-r-br-3 br-w-l-1 br-l-only';
    }
  });
  readonly rightIconClases = computed(() => {
    if (this.dir.direction() == 'ltr') {
      ...
      return 'br-r-tl-3 br-r-bl-3 br-r-only br-w-r-1';
    }
  });
  ...
  readonly inputWrraperClasses = computed(() => {
    let classes =
      'input-wrapper flex-1 d-flex flex-row px-5 fsi-2  position-relative ';
  ...
  readonly inputClases = computed(() => {
    let classes = `br-none flex-1 mx-5 ${this.size()}`;
